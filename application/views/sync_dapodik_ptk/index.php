...
<div class="dashboard-main-body">
    <div class="card mb-4">
        <div class="card-header bg-neutral-300">
            <h6 class="text-dark mb-0">Sinkronisasi Dapodik GTK ke PTK</h6>
        </div>
        <div class="card-body">
            <form action="<?php echo url('sync_dapodik_ptk/fetch') ?>" method="post">
                <div class="row gy-3">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        <label class="form-label fw-semibold">Endpoint GTK</label>
    ...
    <?php if ($run): ?>
        ...
        <div class="row gy-3 mb-4">
            <div class="col-md-3"><div class="card"><div class="card-body"><span class="text-secondary-light d-block">Data GTK Dapodik</span><h5 class="mb-0"><?php echo $run->total_remote ?></h5></div></div></div>
            <div class="col-md-3"><div class="card"><div class="card-body"><span class="text-secondary-light d-block">PTK Baru</span><h5 class="mb-0 text-info-600"><?php echo $summary['baru'] ?></h5></div></div></div>
            <div class="col-md-3"><div class="card"><div class="card-body"><span class="text-secondary-light d-block">Data Berbeda</span><h5 class="mb-0 text-warning-600"><?php echo $summary['berbeda'] ?></h5></div></div></div>
            <div class="col-md-3"><div class="card"><div class="card-body"><span class="text-secondary-light d-block">Data Sama</span><h5 class="mb-0 text-success-600"><?php echo $summary['sama'] ?></h5></div></div></div>
        </div>

        <form action="<?php echo url('sync_dapodik_ptk/apply') ?>" method="post" onsubmit="return confirm('Terapkan data GTK Dapodik yang dipilih ke tabel PTK lokal?')">
            ...
            <div class="card">
                <div class="card-header bg-neutral-100 d-flex flex-wrap justify-content-between align-items-center gap-2">
                    <div>
                        <h6 class="mb-1">Tabel Pembanding GTK Dapodik dan PTK Lokal</h6>
                        <span class="text-secondary-light text-sm">Preview dibuat <?php echo $run->created_at ?>. Data yang sudah sama tidak ditampilkan.</span>
                    </div>
                    <div class="d-flex flex-wrap align-items-center gap-2">
                        <button type="button" class="btn btn-info-100 text-info-600" id="selectAllVisible">Pilih Semua Perubahan</button>
                        ...
                        <table class="table bordered-table" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>NUPTK Lokal</th>
                                    <th>NUPTK Dapodik</th>
                                    <th>Penugasan Lokal</th>
                                    <th>Penugasan Dapodik</th>
                                    <th>Perbedaan</th>
                                    <th>Detail</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($items as $item): ?>
                                    ...
                                    $diff_count = $item->match_status === 'baru' ? count(array_filter($payload, function ($value) { return $value !== ''; })) : count($diff_details);
                                    ?>
                                    <tr>
                                        <td>
                                        ...
                                        <td><?php echo htmlspecialchars($item->nik_lokal ?: '-', ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?php echo htmlspecialchars($item->nik_dapodik ?: '-', ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?php echo htmlspecialchars($item->nuptk_lokal ?: '-', ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?php echo htmlspecialchars($item->nuptk_dapodik ?: '-', ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?php echo htmlspecialchars($item->penugasan_lokal ?: '-', ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?php echo htmlspecialchars($item->penugasan_dapodik ?: '-', ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?php echo $diff_count ?> field</td>
                                        <td><button class="btn btn-sm btn-outline-primary" type="button" data-bs-toggle="modal" data-bs-target="#diffModal-<?php echo $item->id_item ?>">Lihat Detail</button></td>
                                    </tr>
                                <?php endforeach; ?>
                                <?php if (empty($items)): ?>
                                    <tr><td colspan="11" class="text-center text-secondary-light py-4">Tidak ada perubahan. Semua data yang dapat dibandingkan sudah sama.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="modal-dialog modal-xl modal-dialog-scrollable">
                    <div class="modal-content">
                        <div class="modal-header">
                            <div>
                                <h6 class="modal-title mb-1">Detail Pembanding GTK/PTK</h6>
                        ...
                        <div class="modal-body">
                            <div class="table-responsive">
                                <table class="table table-sm bordered-table mb-0">
                                    <thead><tr><th>Field</th><th>Data Lokal</th><th>Data Dapodik</th></tr></thead>
                                    <tbody>
                                        <?php if ($item->match_status === 'baru'): ?>
                                            <?php foreach ($payload as $field => $value): ?>
                                                <?php if ($value === '') continue; ?>
                                                <tr><td><?php echo htmlspecialchars($field, ENT_QUOTES, 'UTF-8') ?></td><td>-</td><td><?php echo htmlspecialchars($value, ENT_QUOTES, 'UTF-8') ?></td></tr>
                                            <?php endforeach; ?>
                                        <?php else: ?>
        ...
        <?php endforeach; ?>
    <?php else: ?>
        <div class="card"><div class="card-body text-center text-secondary-light py-5">Belum ada preview sinkronisasi GTK. Ambil data Dapodik terlebih dahulu untuk melihat tabel pembanding.</div></div>
    <?php endif; ?>
</div>
