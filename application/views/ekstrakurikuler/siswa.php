...
<div class="dashboard-main-body">
    <div class="card mb-4">
        <div class="card-header bg-warning-900 d-flex justify-content-between align-items-center">
            <h6 class="mb-0 text-light">Kelola Anggota Ekstrakurikuler: <?php echo html_escape($ekskul->nama_ekskul) ?></h6>
            <span class="badge bg-neutral-200 text-dark"><?php echo count($siswa_ekskul) ?> Terdaftar</span>
        </div>
        <div class="card-body">
            <div class="siswa-roster-grid">
                ...
                <div>
                    ...
                    <ul id="source-students" class="siswa-dropzone list-group">
                        ...
                        <?php else: ?>
                            <?php foreach ($siswa_belum_masuk as $s) ekskul_siswa_item($s); ?>
                        <?php endif; ?>
                    </ul>
                </div>
                ...
                <div>
                    <h6 class="text-md fw-bold mb-12">Anggota Terdaftar Ekskul</h6>
                    <div class="mb-12">
                    ...
                    <ul id="target-students" class="siswa-dropzone list-group">
                        ...
                        <?php else: ?>
                            <?php foreach ($siswa_ekskul as $s) ekskul_siswa_item($s); ?>
                        <?php endif; ?>
                    </ul>
                </div>
            </div>
    ...
    <!-- Simpan form hidden -->
    <form id="form-roster" action="<?php echo url('ekstrakurikuler/simpan_siswa/' . $ekskul->id_ekskul) ?>" method="post">
        <div id="hidden-inputs">
            <?php foreach ($siswa_ekskul as $s): ?>
        ...
        <div class="card">
            <div class="card-body d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-sm text-secondary-light">Perubahan roster anggota baru akan disimpan setelah Anda menekan tombol simpan.</p>
                </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#search-calon').on('keyup', function () {
            const query = $(this).val().toLowerCase();
            $('#source-students li').each(function () {
        ...
        $('#search-anggota').on('keyup', function () {
            const query = $(this).val().toLowerCase();
